...
void handleCutMotorErrorState() {
    ...
    if (millis() - getLastErrorBlinkTime() > STANDARD_ERROR_BLINK_INTERVAL) {
        ...
        if(newBlinkState) showRedLed(); else turnRedLedOff();
        if(!newBlinkState) showYellowLed(); else turnYellowLedOff();
    ...
    if (getErrorAcknowledged()) {
        onErrorOccurred("Cut motor homing error - acknowledged");

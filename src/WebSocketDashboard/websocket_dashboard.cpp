...
struct ConfigurationData {
    ...
    float ROTATION_SERVO_EARLY_ACTIVATION_DISTANCE;
...
ConfigurationData getDefaultConfiguration() {
    ...
    config.ROTATION_SERVO_EARLY_ACTIVATION_DISTANCE = ROTATION_SERVO_EARLY_ACTIVATION_DISTANCE;
...
void applyConfiguration(const ConfigurationData& config) {
    ...
    ROTATION_SERVO_EARLY_ACTIVATION_DISTANCE = config.ROTATION_SERVO_EARLY_ACTIVATION_DISTANCE;
...
uint32_t calculateChecksum(const ConfigurationData& config) {
    ...
    data = (const uint8_t*)&config.ROTATION_SERVO_EARLY_ACTIVATION_DISTANCE;
    for (size_t i = 0; i < sizeof(config.ROTATION_SERVO_EARLY_ACTIVATION_DISTANCE); i++) checksum += data[i];
